<?php
  ...
  function getCurrentProfilePic () {
    ...
    $stmt = $mysqli->prepare("SELECT filename FROM vpuserprofilepics WHERE userID=?");
	echo $mysqli->error;
	...
	if($stmt->fetch()){
		...
		$_SESSION["profilePic"] = "default.png";
	}
	$stmt->close();
	$mysqli->close();
  }
  
  function addProfilePicData ($fileName) {
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
    $stmt = $mysqli->prepare("INSERT INTO vpuserprofilepics (userID, filename) VALUES (?, ?)");
	echo $mysqli->error;
